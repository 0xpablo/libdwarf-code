...
/*! @brief Get UUID from Mach-O binaries (LC_UUID).
    ...
    Extracts the UUID from Mach-O binaries' LC_UUID load command.
    This is essential for matching binaries with their dSYMs in
    ...
    @return
    Returns DW_DLV_OK on success.
    Returns DW_DLV_NO_ENTRY if no UUID is present.
